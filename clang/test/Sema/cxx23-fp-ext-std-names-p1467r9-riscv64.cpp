// RUN: %clang_cc1 -fsyntax-only -std=c++23 -triple riscv64-linux-gnu -verify -ast-dump %s | FileCheck %s
// Test extended floating-point types on riscv64 where long double == IEEE quad-precision

_Float16 f16_val = 1.0f16;
//CHECK:      VarDecl {{.*}} f16_val '_Float16' cinit
//CHECK-NEXT: FloatingLiteral {{.*}} '_Float16' 1.000000e+00

// On riscv64, long double is IEEE quad-precision (fp128)
...
// Test that float and double can be converted to quad-precision long double
